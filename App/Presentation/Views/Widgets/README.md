...
- `DropletAnalysisWindow` là owned top-level window: owner được resolve thẳng về MainView, transient parent giữ Z-order/vòng đời, và Win32 taskbar style cho phép Windows tạo thumbnail trong cùng nhóm TNH Optima thay vì caption thu nhỏ trên desktop.
...
10. ImageEditor mở Droplet Analysis → truyền source path + MainView owner → helper gán transient owner/taskbar style → grouped preview; close phát `destroyed` để gỡ khỏi danh sách cửa sổ.
